...
import { googleAuthMiddleware } from '../middlewares/authenticateToken';

const router = express.Router();
router.use((req, res, next) => {
  googleAuthMiddleware(req, res, next).catch(next);
});
// Get all tasks
router.get('/', async (req: express.Request, res) => {
  ...
  let query = `SELECT id,title,description,category,due_date as duedate,priority FROM tasks WHERE userid= '${req.user?.sub}'`;
  let values: any = [];
...
router.post('/', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [title, description, category, dueDate, priority, req.user?.sub]
    );
